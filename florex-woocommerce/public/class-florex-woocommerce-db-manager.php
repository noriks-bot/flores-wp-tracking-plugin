...
class Florex_Woocommerce_Db_Manager {
    ...
    public function getUserMeta($hash, $url = null, $utm_source = null, $utm_medium = null, $campaign_name = null, $adset_name = null, $ad_name = null, $campaign_id = null, $adset_id = null, $ad_id = null, $placement = null) {
        ...
        $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}digit_tracking_user_meta WHERE id_user LIKE '%s' AND landing LIKE '%s' AND utm_source LIKE '%s' AND utm_medium LIKE '%s' AND campaign_id LIKE '%s' AND adset_id LIKE '%s' AND ad_id LIKE '%s'", $hash, $url, $utm_source, $utm_medium, $campaign_id, $adset_id, $ad_id ) );
    ...
    public function getOrderJourneyByAttribution($order_id, $attribution) {
        ...
        switch($attribution['attribution']) {
            case 'last':
                $result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}digit_tracking_user_meta WHERE id_order LIKE '%s' AND utm_source != '%s' ORDER BY date DESC LIMIT 1", $order_id, "direct" ) );
            ...
            case 'last_after':
                ...
                $result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}digit_tracking_user_meta WHERE id_order LIKE '%s' AND utm_source != '%s' AND date >= '%s' ORDER BY date ASC LIMIT 1", $order_id, "direct", $cutoff ) );
                if($result) {
                ...
                break;
            default:
                $result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}digit_tracking_user_meta WHERE id_order LIKE '%s' AND utm_source != '%s' ORDER BY date ASC LIMIT 1", $order_id, "direct" ) );
                if (is_null($result) || !empty($wpdb->last_error)) {
                    // Query was empty or a database error occurred
                    $result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}digit_tracking_user_meta WHERE id_order LIKE '%s' ORDER BY date ASC LIMIT 1", $order_id ) );
                }
    ...
    public function getUserAttributions($hash) {
        ...
        $results = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}digit_tracking_user_attribution WHERE id_user LIKE '%s' AND DATE(date) >= %s", $hash, date('Y-m-d', $expired) ) );
    ...
    public function getProductsOptimized($page, $per_page) {
        ...
        $sql = $wpdb->prepare(
            ...
            SELECT 
                ...
                GROUP_CONCAT(o.meta_value ORDER BY o.meta_id SEPARATOR ',') AS old_slugs
            FROM 
                {$wpdb->prefix}posts p 
            LEFT JOIN 
